...
namespace MyClassLibrary
{
    public class ImageClass
    {
        ...
        /// 对图片进行排除异常处理
        /// </summary>
        /// <param name="imgPath">源图片绝对路径</param>
        private static Graphics PixelFormatIndexedFormat(string imgPath)
        {
            Graphics g = null;
        ...
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, string mode)
        {
            ...
            if (IsPixelFormatIndexed(bitmap.PixelFormat))
            {
                ...
                gx.DrawImage(originalImage, new System.Drawing.Rectangle(0, 0, towidth, toheight), new System.Drawing.Rectangle(x, y, ow, oh), System.Drawing.GraphicsUnit.Pixel);

                try
                {
                    //以jpg格式保存缩略图
                    bmp.Save(thumbnailPath, System.Drawing.Imaging.ImageFormat.Jpeg);
                }
        ...
        public static string ImageWatermark(string path, string waterpath, string location)
        {
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg")
            {
                ...
                if (IsPixelFormatIndexed(img.PixelFormat))
                {
                    ...
                    ArrayList loca = GetLocation(location, bmp, waterimg);
                    gx.DrawImage(waterimg, new Rectangle(int.Parse(loca[0].ToString()), int.Parse(loca[1].ToString()), waterimg.Width, waterimg.Height));
                    waterimg.Dispose();
                    gx.Dispose();
        ...
        public static string LetterWatermark(string path, int size, string letter, Color color, string location)
        {
            ...
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg")
            {
                ...
                if (IsPixelFormatIndexed(img.PixelFormat))
                {
                    ...
                    ArrayList loca = GetLocation(location, bmp, size, letter.Length);
